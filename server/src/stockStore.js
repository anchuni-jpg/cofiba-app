...
// para llevar control:
//   - se agotó (pasó a 0) o se repuso (estaba a 0 y ya no)
//   - cruzó el umbral de 10 cajas que ya usa STOCK/STOCK BAJO
//   - bajó al menos un 50% de golpe, aunque siga por encima de esas 10 cajas
//
// Igual que novedadesStore.js: vive en el SERVIDOR, no en el dispositivo del
// cliente, para que cualquier cuenta vea los mismos cambios reales desde el
// primer momento sin depender de una comparación local previa.
...
export function actualizarDesdeIndice(productos) {
  ...
  if (nuevosCambios.length) {
    cambios = [...nuevosCambios, ...cambios].filter((c) => ahora - c.fecha <= QUINCE_DIAS_MS).slice(0, LIMITE);
    guardarEnDisco();
  } else if (huboSnapshotPrevio) {
    // El snapshot en sí (stock de ahora) siempre se actualiza aunque no
    // hubiera cambios "notables" que anotar en la lista.
    guardarEnDisco();
  }
}
...
export function cambiosRecientes() {
  const ahora = Date.now();
  return cambios.filter((c) => ahora - c.fecha <= QUINCE_DIAS_MS);
}
